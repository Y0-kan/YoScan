...
class BaiduSpider:
    def __init__(self):
        ...
        self.wds = ['inurl:admin', 'inurl:login', 'inurl:system', 'inurl:register', 'inurl:upload', '后台', '系统', '登录','管理']
    ...
    def keyword(self, wd, page=1):
        ...
        try:
            ...
            bd_link_titles = re.findall('<div class="c-tools" id="\S*" data-tools=\'\{"title":"(.*)","url":"(.*)"\}\'>', res.text)
            print('[{}] {}'.format(res.status_code, url))
            print(bd_link_titles)
...
if __name__ == '__main__':
    ...
    link = []
    baidu = BaiduSpider()
    subdomain,link = baidu.run_subdomain('test.com')
